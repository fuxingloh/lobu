...
export interface RepositoryModalOptions {
  ...
  getGitHubUserInfo?: (userId: string) => Promise<{ token: string | null; username: string | null }>;
}
...
export async function openRepositoryModal({
  ...
  try {
    ...
    let isAdmin = false;
    if (checkAdminStatus && metadata.channel_id && !metadata.channel_id.startsWith('D')) {
      try {
        ...
        logger.warn(`Could not check admin status for ${userId} in ${metadata.channel_id}`);
      }
    }
    ...
    if (!githubUser.token) {
      ...
      blocks.push({
        ...
        text: {
          type: "mrkdwn",
          text: isAdmin && metadata.channel_id && !metadata.channel_id.startsWith('D')
      ...
      if (isAdmin && metadata.channel_id && !metadata.channel_id.startsWith('D')) {
        blocks.push({
          type: "context",
          elements: [{
            type: "mrkdwn",
    ...
    await client.views.open({
      ...
      view: {
        ...
        submit: githubUser.token ? { type: "plain_text", text: "Select" } : undefined,
