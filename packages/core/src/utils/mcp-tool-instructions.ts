...
export function buildMcpToolInstructions(
  ...
  const lines: string[] = [
    ...
    "",
    "**Tool approval:** Some tools require user approval before use. If a tool call returns HTTP 403, the tool requires a permission grant. Use Configure to send the user a settings link where they can approve access, then wait and retry after approval.",
    "",
  ];
