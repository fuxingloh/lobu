...
First boot pays the install cost. After that packages are cached on the workspace volume (Docker) or PVC (Kubernetes). Survives container restarts and scale-to-zero. For complex environments you can point at a [Nix flake](https://nixos.wiki/wiki/Flakes) instead:
