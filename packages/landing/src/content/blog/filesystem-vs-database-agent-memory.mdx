...
- If `MEMORY_URL` is not set, Lobu uses `@openclaw/native-memory`, so memory stays local to that workspace.
- If `MEMORY_URL` is set, Lobu uses `@lobu/owletto-openclaw` when that plugin is installed, otherwise it falls back to native memory.
