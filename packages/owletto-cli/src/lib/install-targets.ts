...
/** Run an owletto subcommand using the current process entry point (works via npx). */
function runOwlettoCli(args: string[], timeoutMs = 30_000): string {
...
function upsertCursorMcpServer(mcpUrl: string): 'added' | 'updated' | 'unchanged' {
...
export const INSTALL_TARGETS: InstallTarget[] = [
  {
    ...
    async configure(mcpUrl) {
      try {
        runCommand('claude', ['mcp', 'add', '--transport', 'http', 'owletto', mcpUrl]);
  ...
  },
  {
    ...
    async configure(mcpUrl) {
      try {
        runCommand('gemini', ['mcp', 'add', '--transport', 'http', 'owletto', mcpUrl]);
        return { status: 'configured', message: 'MCP server added' };
      } catch (e) {
        return { status: 'failed', message: (e as Error).message };
      }
  ...
  },
  {
    ...
    async configure(mcpUrl) {
      try {
        runCommand('openclaw', ['plugins', 'install', 'owletto-openclaw-plugin']);
      } catch (e) {
        return { status: 'failed', message: `Plugin install failed: ${(e as Error).message}` };
      }
      try {
        runOwlettoCli(['configure', '--url', mcpUrl]);
      } catch (e) {
        return { status: 'failed', message: `Configure failed: ${(e as Error).message}` };
      }
      try {
        runOwlettoCli(['health', '--url', mcpUrl]);
        return { status: 'configured', message: 'Plugin installed and verified' };
      } catch (e) {
        return { status: 'failed', message: `Health check failed: ${(e as Error).message}` };
      }
    },
  },
  {
    ...
    async configure(mcpUrl) {
      try {
        ...
        if (action === 'unchanged') {
          return { status: 'configured', message: 'MCP server already configured in Cursor' };
        }
        return {
          status: 'configured',
          message:
