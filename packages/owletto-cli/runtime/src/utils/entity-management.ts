...
export async function getEntity(
  ...
  const result = await sql<CreatedEntity>`
    SELECT
      ...
      (SELECT COUNT(*) FROM current_event_records ev WHERE e.id = ANY(ev.entity_ids)) as total_content,
      (
...
export async function listEntities(
  ...
  const baseQuery = `
    FROM entities e
    LEFT JOIN entities pe ON e.parent_id = pe.id
    LEFT JOIN LATERAL (SELECT COUNT(*) as cnt FROM current_event_records ev WHERE e.id = ANY(ev.entity_ids)) tc ON true
    LEFT JOIN LATERAL (
