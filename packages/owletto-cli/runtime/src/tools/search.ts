...
export async function search(
  ...
  // Helper to run content search in parallel (only when we have a text query)
  const contentSearchPromise =
    includeContent && args.query
      ? fetchContentSnippets(args.query, ctx.organizationId, contentLimit, env).catch((err) => {
          logger.warn(
...
async function fetchTopEntitiesByType(
  ...
  const rows = await sql`
    ...
    ORDER BY (SELECT COUNT(*) FROM current_event_records ev WHERE entities.id = ANY(ev.entity_ids)) DESC
    LIMIT 30
  `;
...
const ENTITY_SELECT_COLUMNS = `
  ...
  COALESCE((SELECT COUNT(*) FROM current_event_records ev WHERE e.id = ANY(ev.entity_ids)), 0) as content_count,
  COALESCE((
