...
async function handleConnect(
  ...
  if (!allowed) {
    ...
    try {
      clientSocket.write(
        `HTTP/1.1 403 Domain not allowed: ${hostname}. Use Configure with prefillGrants to request access.\r\nContent-Type: text/plain\r\n\r\n403 Forbidden - Domain not allowed: ${hostname}. Use Configure with prefillGrants to request access.\r\n`
      );
      clientSocket.end();
    } catch {
      // Client may have already disconnected
